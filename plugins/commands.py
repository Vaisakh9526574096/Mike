...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        ...
        await message.reply(STOP_MESSAGE.format(user=message.from_user.mention if message.from_user else message.chat.title, bot=temp.B_LINK), reply_markup=InlineKeyboardMarkup(buttons), disable_web_page_preview=True)                    
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        if message.command[1] != "subscribe":
            try:
                ...
                btn.append([InlineKeyboardButton("✅️ Done", callback_data=f"{pre}#{file_id}")])
            except (IndexError, ValueError):
                btn.append([InlineKeyboardButton("✅️ Done", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")])
...
async def send_msg(bot, message):
    if message.reply_to_message:
        ...
        if success:
            await message.reply_text(f"<b>Your message has been send to {user.mention}.</b>")
        else:
            await message.reply_text("an error occured")
    else:
        await message.reply_text("Incomplete command!")
